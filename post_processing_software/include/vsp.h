 ...
 **/
    ...
    void getUndistortUnrollingChessBoard(
        int32_t frame,
        const std::vector<_Tx> &src, std::vector<_Tx> &dst)
    {
        ...
        for (const auto &el : src) //(int j = 0; j <= division_y; ++j)
        {
            ...
            Eigen::Quaternion<double> slerpedAngleQuaternion;
            if (exposureTimingInEachRow >= 0.0)
            {
                slerpedAngleQuaternion = currAngleQuaternion.slerp(exposureTimingInEachRow, nextAngleQuaternion);
            }
            else
            ...
            // for (int i = 0; i <= division_x; ++i)
            {
                ...
                //後々の行列演算に備えて、画像上の座標を同次座標で表現しておく。(x座標、y座標,1)T
                Eigen::Vector3d p;
                p << (u - camera_info_.cx_) / camera_info_.fx_, (v - camera_info_.cy_) / camera_info_.fy_, 1.0; // Homogenious coordinate
                //2
                ...
                //                src.push_back(x2*fx*zoom/camera_info_.width_*2.0);
                //                src.push_back(y2*fy*zoom/camera_info_.height_*2.0);
                // map_x(j, i) = x2 * camera_info_.fx_ * zoom / camera_info_.width_ * 2.0;
                // map_y(j, i) = y2 * camera_info_.fy_ * zoom / camera_info_.height_ * 2.0;
                dst.push_back(_Tx(
                    x2 * camera_info_.fx_ + camera_info_.cx_,
                    y2 * camera_info_.fy_ + camera_info_.cy_));
            }
        }
        return;
    }
