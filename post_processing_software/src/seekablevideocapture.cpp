...
//    int32_t lengthOfPrefetch = position - capture.get(CV_CAP_PROP_POS_FRAMES) + buffLen/2;
    ...
    if((position + buffLen/2) < capture.get(cv::CAP_PROP_FRAME_COUNT)){//動画の最後に達していない
        lengthOfPrefetch  = position - capture.get(cv::CAP_PROP_POS_FRAMES) + buffLen/2;
    }else{//動画の最後に達した
        lengthOfPrefetch = 0;
    }
...
//        std::cout << "buff.size()" << buff.size() << std::endl;
    ...
    int32_t readFramePositionOfDeque = position - (capture.get(cv::CAP_PROP_POS_FRAMES)-buff.size());
